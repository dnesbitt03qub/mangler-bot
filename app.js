...
try {
  if (ngramMangling) {
      // Read in bigrams and freq maps from file      
      let bigramString = fs.readFileSync(BIGRAM_PATH);
      let freqString = fs.readFileSync(FREQ_PATH);
      console.log('Parsing objects read from file');
      bigrams = JSON.parse(bigramString);
      freqString = JSON.parse(freqString);
      console.log('Parsed objects read from file');
